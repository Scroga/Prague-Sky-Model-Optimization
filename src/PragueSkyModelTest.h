...
template<class SkyModel>
SkyModel::Vector3 pixelToDirection(int x, int y, int resolution, View view) {
				...
				if (denom > 2.0) {
								...
								if (view == View::SideFacingFisheye) {
												return SkyModel::Vector3(-2.0 * scaledy / denom, -(denom - 2.0) / denom, -2.0 * scaledx / denom);
								}
...
SkyModel::Vector3 spectrumToRGB(const Spectrum& spectrum) {
				...
				return SkyModel::Vector3(rgb.x, rgb.y, rgb.z);
}
				...
				std::for_each(std::execution::par,
								...
								&outResult](auto&& x) {
												for (int y = 0; y < resolution; y++) {
																...
																// projection.
																const SkyModel::Vector3 viewDir = pixelToDirection<SkyModel>(x, y, resolution, view);
																...
																const SkyModel::Parameters params = model.computeParameters(viewPoint,
																				viewDir,
																				elevation,
																				azimuth,
																				visibility,
																...
																const SkyModel::Vector3 rgb = spectrumToRGB<SkyModel>(spectrum);
