...
namespace Realtime {
				...
				using Vector3 = RealtimePragueSkyModel::Vector3;

				/// Converts a 16-bit half-precision floatin-point value into a C++ double.
				...
				inline double dot(const Vector3& v1, const Vector3& v2) noexcept {
								return v1.x * v2.x + v1.y * v2.y + v1.z * v2.z;
				}

				inline double magnitude(const Vector3& vector) noexcept {
								return std::sqrt(dot(vector, vector));
				}

				inline Vector3 normalize(const Vector3& vector) noexcept {
								const double len = magnitude(vector);
				...
				inline double lerp(const double from, const double to, const double factor) {
								return (1.0 - factor) * from + factor * to;
				}

				inline double nonlerp(const double a, const double b, const double w, const double p) {
				...
				void RealtimePragueSkyModel::readRadiance(FILE* handle, const double singleVisibility) {
								// Read metadata.
				...
				// Initialization
				/////////////////////////////////////////////////////////////////////////////////////
				void RealtimePragueSkyModel::initialize(const std::string& filename, const double singleVisibility) {
				...
				RealtimePragueSkyModel::AvailableData RealtimePragueSkyModel::getAvailableData() const {
								if (!initialized) {
				...
				RealtimePragueSkyModel::Parameters RealtimePragueSkyModel::computeParameters(
								...
								// TODO: this can be eliminated if the sun didn't change its position
								// Direction to sun
				...
				inline double evalPL(const std::vector<float>::const_iterator coefs, const double factor) {
								return (double(coefs[1]) - double(coefs[0])) * factor + double(coefs[0]);
				}

				// Returns the pointer to the first float of the needed coefficient block.
				std::vector<float>::const_iterator RealtimePragueSkyModel::getCoefficients(
				...
				RealtimePragueSkyModel::InterpolationParameter RealtimePragueSkyModel::getInterpolationParameter(
				...
				double RealtimePragueSkyModel::reconstruct(
				...
				double RealtimePragueSkyModel::evaluateModel(const Parameters& params,
				...
				double RealtimePragueSkyModel::skyRadiance(const Parameters& params, const double wavelength) const {
								if (!initialized) {
												throw NotInitializedException();
								}
				...
				double RealtimePragueSkyModel::sunRadiance(const Parameters& params, const double wavelength) const {
								if (!initialized) {
												throw NotInitializedException();
								}
								...
								const double tau = RealtimePragueSkyModel::transmittance(params, wavelength, std::numeric_limits<double>::max());
				...
				double RealtimePragueSkyModel::polarisation(const Parameters& params, const double wavelength) const {
								if (!initialized) {
												throw NotInitializedException();
								}
				...
				std::vector<float>::const_iterator RealtimePragueSkyModel::getCoefficientsTrans(const int visibility,
								const int altitude,
								const int wavelength) const {
				...
				std::vector<float>::const_iterator RealtimePragueSkyModel::getCoefficientsTransBase(const int altitude,
								const int a,
								const int d) const {
				...
				RealtimePragueSkyModel::InterpolationParameter RealtimePragueSkyModel::getInterpolationParameterTrans(const double value,
				...
				RealtimePragueSkyModel::TransmittanceParameters RealtimePragueSkyModel::toTransmittanceParams(const double theta,
				...
				double RealtimePragueSkyModel::reconstructTrans(const int                     visibilityIndex,
				...
				double RealtimePragueSkyModel::interpolateTrans(const int                     visibilityIndex,
				...
				double RealtimePragueSkyModel::transmittance(const Parameters& params,
								const double      wavelength,
